...
public class ServerThread implements Runnable {
	...
	public void run() {
		try {
			...
			while ((content = mReader.readLine()) != null) {
				...
				// 包头格式为：动作名称,设备编号,昵称(唯一标识，即用户名),时间,对方设备编号,密码|消息主体
				...
				if (action.equals("REGISTER")) {
					register(splitArray[1], splitArray[2], splitArray[5]);
				} else if (action.equals("LOGIN")) {
					login(splitArray[1], splitArray[2], splitArray[3], splitArray[5]);
	...
	private void register(String deviceId, String nickName, String password) throws Exception {
		if (DBUtil.createAccount(DBUtil.getConn(), nickName, password)) {
			PrintStream ps = new PrintStream(mSocket.socket.getOutputStream());
			ps.println("REGISTER,SUCCESS,|");
			System.out.println("回复数据：REGISTER,SUCCESS,|");
		} else {
			PrintStream ps = new PrintStream(mSocket.socket.getOutputStream());
			ps.println("REGISTER,FAILED,|");
			System.out.println("回复数据：REGISTER,FAILED,|");
		}
	}
	
	private void login(String deviceId, String nickName, String loginTime, String password) throws Exception {
		if (!DBUtil.loginAccount(DBUtil.getConn(), nickName, password)) {
			PrintStream ps = new PrintStream(mSocket.socket.getOutputStream());
			ps.println("LOGIN,FAILED,|");
			System.out.println("回复数据：LOGIN,FAILED,|");
			return;
		} else {
			PrintStream ps = new PrintStream(mSocket.socket.getOutputStream());
			ps.println("LOGIN,SUCCESS,|");
			System.out.println("回复数据：LOGIN,SUCCESS,|");
